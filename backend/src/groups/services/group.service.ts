...
export class GroupService implements IGroupService {
  ...
  async transferGroupOwner({
    ...
    if (group.owner.id !== userId)
      throw new HttpException('Group Owner Transfer Exception : Insufficient Permissions',HttpStatus.BAD_REQUEST);
    if (group.owner.id === newOwnerId)
      throw new HttpException(
        'Group Owner Transfer Exception : Cannot Transfer Owner to yourself',HttpStatus.BAD_REQUEST
      );
    const newOwner = await this.userService.findUser({ id: newOwnerId });
    if (!newOwner) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
